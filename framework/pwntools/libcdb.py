...
from pwntools.util.fiddling import enhex
from pwntools.util.hashes import sha1filehex, sha256filehex, md5filehex
from pwntools.util.misc import read
from pwntools.util.misc import which
from pwntools.util.misc import write
from pwntools.util.web import wget
...
def provider_libcdb(hex_encoded_id, hash_type):
    ...
    try:
        while not data.startswith(b'\x7fELF'):
            ...
            if not data:
                log.warn_once(
                    "Could not fetch libc for %s %s from libcdb",
                    hash_type,
                    hex_encoded_id)
                break
        ...
        log.warn_once(
            "Failed to fetch libc for %s %s from libcdb: %s",
            hash_type,
            hex_encoded_id,
            e)
    return data
...
def provider_libc_rip(hex_encoded_id, hash_type):
    ...
    if not libc_match:
        log.warn_once(
            "Could not find libc info for %s %s on libc.rip",
            hash_type,
            hex_encoded_id)
        return None
    ...
    if len(libc_match) > 1:
        log.debug(
            "Received multiple matches. Choosing the first match and discarding the others.")
    ...
    if not data:
        log.warn_once(
            "Could not fetch libc binary for %s %s from libc.rip",
            hash_type,
            hex_encoded_id)
        return None
    return data
...
def provider_local_system(hex_encoded_id, hash_type):
    ...
    if not os.path.exists(shell_path):
        log.debug(
            'Shell path %r does not exist. Skipping local system libc matching.',
            shell_path)
        return None
    local_libc = ELF(shell_path, checksec=False).libc
    if not local_libc:
        log.debug(
            'Cannot lookup libc from shell %r. Skipping local system libc matching.',
            shell_path)
        return None
...
def provider_local_database(hex_encoded_id, hash_type):
    ...
    log.debug(
        "Searching local libc database, %s: %s",
        hash_type,
        hex_encoded_id)
...
def query_local_database(params):
    ...
    for symbol_file in sorted(localdb.rglob(
            "*.symbols"), key=lambda x: x.as_posix()):
        ...
        for name, addr in query_syms.items():
            ...
            if libc_syms.get(name) and (
                    libc_syms.get(name) & 0xfff) == (addr & 0xfff):
                matched += 1
            else:
        ...
        if matched == len(query_syms):
            ...
            libs_url = read(symbol_file.parent / ("%s.url" %
                            libs_id)).decode().strip()
            res.append(
                _pack_libs_info(
                    libc_path,
                    libs_id,
...
def search_by_hash(hex_encoded_id, hash_type='build_id',
                   unstrip=True, offline_only=False):
    ...
    # Run through all available libc database providers to see if we have a
    # match.
    ...
    if not data:
        log.warn_once(
            "Could not find libc for %s %s anywhere",
            hash_type,
            hex_encoded_id)
...
def _search_debuginfo_by_hash(base_url, hex_encoded_id):
    ...
    cache, cache_valid = _check_elf_cache(
        'libcdb_dbg', hex_encoded_id, 'build_id')
    if cache_valid:
        return cache
    ...
    try:
        ...
        log.warn_once(
            "Failed to fetch libc debuginfo for build_id %s from %s: %s",
            hex_encoded_id,
            base_url,
            e)
    ...
    if not data or not data.startswith(b'\x7FELF'):
        log.warn_once(
            "Could not fetch libc debuginfo for build_id %s from %s",
            hex_encoded_id,
            base_url)
        return None
...
def unstrip_libc(filename):
    ...
    if not libc.buildid:
        log.warn_once(
            'Given libc does not have a buildid. Cannot look for debuginfo to unstrip.')
        return False

    if libc.debuginfo:
        log.debug(
            'Given libc already contains debug information. Skipping unstrip.')
        return True
    ...
    for server_url in DEBUGINFOD_SERVERS:
        ...
        log.warn_once(
            'Couldn\'t find debug info for libc with build_id %s on any debuginfod server.',
            enhex(
                libc.buildid))
        return False
...
def _extract_tarfile(cache_dir, data_filename, tarball):
    ...
    import tarfile
    # Handle zstandard compression, since tarfile only supports gz, bz2, and
    # xz.
    ...
    if six.PY2 and data_filename.endswith('.xz'):
        ...
        import subprocess
        try:
            uncompressed_tarball = subprocess.check_output(
                ['xz', '--decompress', '--stdout', tarball.name])
    ...
    with tarfile.open(fileobj=tarball) as tar_file:
        ...
        for member in tar_file.getmembers():
            ...
            if libc_name == 'libc.so.6' or (libc_name.startswith(
                    'libc') and libc_name.endswith('.so')):
                lib_dir = os.path.dirname(member.name)
                break
        else:
            log.error(
                'Couldn\'t find library folder containing the libc in the archive.')
...
def _extract_debfile(cache_dir, package_filename, package):
    ...
    if six.PY2:
        if not which('ar'):
            log.error(
                'Missing command line tool "ar" to extract .deb archive. Please install "ar" first.')

        import atexit
        import shutil
        import subprocess

        # Use mkdtemp instead of TemporaryDirectory because the latter is not
        ...
        with tempfile.NamedTemporaryFile(mode='wb', dir=tempdir) as debfile:
            ...
            debfile.flush()
            try:
                files_in_deb = subprocess.check_output(
                    ['ar', 't', debfile.name]).split(b'\n')
            except subprocess.CalledProcessError:
                log.error('Failed to list files in .deb archive.')
            [data_filename] = filter(
                lambda f: f.startswith(b'data.tar'), files_in_deb)

            try:
                subprocess.check_call(
                    ['ar', 'x', debfile.name, data_filename], cwd=tempdir)
            except subprocess.CalledProcessError:
        ...
        try:
            data_filename = next(
                filter(
                    lambda f: f.name.startswith(b'data.tar'),
                    ar_file.infolist())).name.decode()
...
def _find_libc_package_lib_url(libc):
    ...
    import re
    version = re.search(
        br'GNU C Library \(Ubuntu E?GLIBC ([^\)]+)\)',
        libc.data)
...
def _handle_multiple_matching_libcs(matching_libcs):
    ...
    from pwntools.ui import options
    ...
    selected_index = options(
        "Select the libc version to use:", [
            libc['id'] for libc in matching_libcs])
    return matching_libcs[selected_index]


def search_by_symbol_offsets(symbols, select_index=None,
                             unstrip=True, return_as_list=False, offline_only=False):
    """
    ...
    if len(matching_list) == 1:
        return search_by_build_id(
            matching_list[0]['buildid'], unstrip=unstrip, offline_only=offline_only)
    ...
    if select_index is not None:
        if select_index > 0 and select_index <= len(matching_list):
            return search_by_build_id(
                matching_list[select_index - 1]['buildid'], unstrip=unstrip, offline_only=offline_only)
        else:
            log.error(
                'Invalid selected libc index. %d is not in the range of 1-%d.',
                select_index,
                len(matching_list))
            return None
    ...
    return search_by_build_id(
        selected_libc['buildid'], unstrip=unstrip, offline_only=offline_only)
...
def get_build_id_offsets():
    ...
    return {
        # $ check_arch 80386
        # 181 Displaying notes found at file offset 0x00000174 with length
        # 0x00000024:
        'i386': [0x174, 0x1b4, 0x1d4],
        # $ check_arch "ARM, EABI5"
        # 69 Displaying notes found at file offset 0x00000174 with length
        # 0x00000024:
        'arm': [0x174],
        'thumb': [0x174],
        # $ check_arch "ARM aarch64"
        # 1 Displaying notes found at file offset 0x00000238 with length
        # 0x00000024:
        'aarch64': [0x238],
        # $ check_arch "x86-64"
        #       6 Displaying notes found at file offset 0x00000174 with length 0x00000024:
        # 82 Displaying notes found at file offset 0x00000270 with length
        # 0x00000024:
        'amd64': [0x270, 0x174, 0x2e0, 0x370],
        # $ check_arch "PowerPC or cisco"
        # 88 Displaying notes found at file offset 0x00000174 with length
        # 0x00000024:
        'powerpc': [0x174],
        # $ check_arch "64-bit PowerPC"
        # 30 Displaying notes found at file offset 0x00000238 with length
        # 0x00000024:
        'powerpc64': [0x238],
        # $ check_arch "SPARC32"
        # 32 Displaying notes found at file offset 0x00000174 with length
        # 0x00000024:
        'sparc': [0x174],
        # $ check_arch "SPARC V9"
        # 33 Displaying notes found at file offset 0x00000270 with length
        # 0x00000024:
        'sparc64': [0x270]
    }.get(context.arch, [])
