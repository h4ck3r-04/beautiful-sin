...
def query_local_database(params):
  ...
  for symbol_file in sorted(localdb.rglob("*.symbols"),
...
def unstrip_libc(filename):
  ...
  Examples:
      ...
      '0x219c80'
      >>> unstrip_libc(pwntools.data.elf.get('test-x86'))
      False
...
def _extract_tarfile(cache_dir, data_filename, tarball):
  ...
  with tarfile.open(fileobj=tarball) as tar_file:
    ...
    for member in tar_file.getmembers():
      ...
      if libc_name == 'libc.so.6' or (
              libc_name.startswith('libc') and libc_name.endswith('.so')):
  ...
  if six.PY2:
    ...
    with tempfile.NamedTemporaryFile(mode='wb', dir=tempdir) as debfile:
      ...
      [data_filename] = filter(
          lambda f: f.startswith(b'data.tar'),
          files_in_deb)
