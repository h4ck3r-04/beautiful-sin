...
from pwntools.util.misc import read, write
from pwntools.util.packing import _encode, _decode
...
class SSHPath(PosixPath):
    ...
    def __init__(self, path, ssh=None):
        ...
        if self.ssh is None:
            raise ValueError(
                'SSHPath requires an ssh session.  Provide onee or set context.ssh_session.')
...
# ---------------------------------- PUREPATH ----------------------------
    ...
    def __repr__(self):
        return "{}({!r}, ssh={!r})".format(
            self.__class__.__name__, self.as_posix(), self.ssh)
