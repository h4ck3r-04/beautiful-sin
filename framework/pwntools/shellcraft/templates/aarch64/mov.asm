<%
  ...
  from pwntools.context import context as ctx # Ugly hack, mako will not let it be called context
  from pwntools.log import getLogger
  from pwntools.util.lists import group
  from pwntools.util.packing import p16, u16, pack, unpack
  from pwntools.util.fiddling import xor_pair
  from pwntools.shellcraft import pretty
  from pwntools.shellcraft.registers import aarch64 as regs
  import six
  log = getLogger('pwntools.shellcraft.arm.mov')
%>
<%page args="dst, src"/>
<%docstring>
Move src into dest.
...
`context.arch` to `'arm'` and use :func:`pwntools.constants.eval` to evaluate the
