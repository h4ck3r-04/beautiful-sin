<%
    ...
    from pwntools.util.packing import flat, unpack
...
If src is a string, then we try to evaluate using :func:`pwntools.constants.eval`
...
Example:
    ...
    >>> print(pwntools.shellcraft.push(0).rstrip())
        /* push 0 */
        mov  x14, xzr
        str x14, [sp, #-16]!
    >>> print(pwntools.shellcraft.push(9).rstrip())
        /* push 9 */
        mov  x14, #9
        str x14, [sp, #-16]!
    >>> print(pwntools.shellcraft.push(256).rstrip())
        /* push 0x100 */
        mov  x14, #256
        str x14, [sp, #-16]!
    >>> print(pwntools.shellcraft.push('SYS_execve').rstrip())
        /* push SYS_execve (0xdd) */
        mov  x14, #221
        str x14, [sp, #-16]!
    >>> print(pwntools.shellcraft.push('SYS_sendfile').rstrip())
