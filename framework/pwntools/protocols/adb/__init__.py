...
from pwntools.tubes.listen import listen
from pwntools.tubes.process import process
from pwntools.tubes.remote import remote
from pwntools.util.lists import group
from pwntools.util.misc import size
from pwntools.util.packing import p32
from pwntools.util.proc import pidof
...
class AdbClient(Logger):
    ...
    def c(self):
        ...
        if not self._c:
            try:
                # Squelch the first '[ERROR] Could not connect to localhost on
                # port 5037'
                level = self.level
                with context.quiet:
    ...
    def version(self):
        ...
        .. doctest::
           :skipif: skip_android
            ...
            >>> pwntools.protocols.adb.AdbClient().version() # doctest: +SKIP
            (4, 36)
        """
    ...
    def transport(self, serial=None, try_again=True):
        ...
        .. doctest::
           :skipif: skip_android
            ...
            >>> pwntools.protocols.adb.AdbClient().transport()
        """
        ...
        if self.send(msg) == FAIL:
            ...
            if serial:
                self.error(
                    "Could not set transport to %r (%s)" %
                    (serial, err))
            else:
    ...
    def execute(self, argv):
        ...
        Returns:
            A :class:`pwntools.tubes.tube.tube` which is connected to the process.
    ...
    def wait_for_device(self, serial=''):
        ...
        if response != OKAY:
            ...
            self.error(
                "An error occurred while trying to wait for device with serial %r (%r)" %
                (serial, response))
        ...
        if response != OKAY:
            if response == FAIL:
                response = self.recvl().decode('utf-8')
            self.error(
                "An error occurred while waiting for device with serial %r (%r)" %
                (serial, response))
    ...
    def _sync(fn):
        ...
        def wrapper(self, *a, **kw):
            if self.send('sync:') == FAIL:
                self.error(
                    "An error occurred while trying to use SYNC API (%r)" %
                    self.recvl().decode('utf-8'))
            return fn(self, *a, **kw)
        return wrapper
    ...
    def list(self, path):
        ...
        .. doctest::
           :skipif: skip_android
            ...
            ...
            PwntoolsException: Cannot list directory '/does/not/exist': Does not exist
        """
        st = self.stat(path)
        ...
        if not stat.S_ISDIR(st['mode']):
            log.error(
                "Cannot list directory %r: Path is not a directory" %
                path)
    ...
    def stat(self, path):
        ...
        .. doctest::
           :skipif: skip_android
            ...
            >>> pwntools.protocols.adb.AdbClient().stat('/proc')           == expected
            True
            >>> pwntools.protocols.adb.AdbClient().stat('/does/not/exist') is None
            True
        """
        if isinstance(path, six.text_type):
            path = path.encode('utf-8')
    ...
    def write(self, path, data, mode=0o755, timestamp=None, callback=None):
        ...
        return self._write(path, data, mode=0o755,
    ...
    @_with_transport
    def forward(self, device, host_proto, host_port,
                device_proto, device_port):
        self.send('host:forward:%s:%s;%s:%s' %
                  (host_proto, host_port, device_proto, device_port))
        self.c.recvall()
