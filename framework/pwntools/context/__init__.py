...
from pwntools.config import register_config
...
class ContextType(object):
    ...
    def native(self):
        ...
        with context.local(arch=platform.machine()):
            ...
            if arch in ('i386', 'amd64') and platform_arch in (
                    'i386', 'amd64'):
                return True
    ...
    def arch(self, arch):
        ...
        # We have to make sure that x86_64 appears before x86 for this to work
        # correctly.
        ...
        try:
            ...
            raise AttributeError(
                'AttributeError: arch (%r) must be one of %r' %
                (arch, sorted(
                    self.architectures)))
    ...
    def log_level(self, value):
        ...
        try:
            ...
            level_names = filter(
                lambda x: isinstance(
                    x, str), logging._levelNames)
        permitted = sorted(level_names)
        raise AttributeError(
            'log_level must be an integer or one of %r' %
            permitted)
    ...
    def log_file(self, value):
        ...
        if isinstance(value, (bytes, six.text_type)):
            ...
            from pwntools.util.packing import _need_text
    ...
    def local_libcdb(self, path):
        ...
        if not os.path.isdir(path):
            raise AttributeError(
                "'%s' does not exist, please download libc-database first" %
                path)
    ...
    def signed(self, signed):
        ...
        if isinstance(signed, str):
            raise AttributeError(
                'signed must be one of %r or a non-string truthy value' %
                sorted(
                    self.signednesses))
    ...
    def proxy(self, proxy):
        ...
        if not isinstance(proxy, Iterable):
            raise AttributeError(
                'proxy must be a string hostname, or tuple of arguments for socks.set_default_proxy')
    ...
    def device(self, device):
        ...
        if isinstance(device, Device):
            ...
            raise AttributeError(
                "device must be either a Device object or a serial number as a string")
    ...
    def cyclic_size(self, size):
        ...
        if size > self.bytes:
            raise AttributeError(
                "cyclic pattern size cannot be larger than word size")

        return size

    @_validator
    def ssh_session(self, shell):
        from pwntools.tubes.ssh import ssh
    ...
    def setter(*a, **kw):
        with context.local(**{k: kw.pop(k) for k, v in tuple(kw.items()) if isinstance(getattr(ContextType, k, None), property)}):
            ...
            if (arch == 'i386' and bits != 32) \
                ...
                raise AttributeError(
                    "Invalid arch/bits combination: %s/%s" %
                    (arch, bits))

            if arch in ('i386', 'amd64') and endian == 'big':
                raise AttributeError(
                    "Invalid arch/endianness combination: %s/%s" %
                    (arch, endian))
...
def update_context_defaults(section):
    ...
    for key, value in section.items():
        ...
        if isinstance(default, six.string_types +
                      six.integer_types + (tuple, list, dict)):
            value = safeeval.expr(value)
        else:
            log.warn(
                "Unsupported configuration option %r in section %r" %
                (key, 'context'))
        ...
        try:
            ...
            log.warn(
                "Could not set context.%s=%s via pwn.conf (%s)",
                key,
                section[key],
                e)
            continue
