...
from pwntools.device import Device
...
def devices(serial=None):
    ...
    for line in lines.splitlines():
        # Skip the first 'List of devices attached' line, and the final empty
        # line.
...
def with_device(f):
    ...
    def wrapper(*a, **kw):
        ...
        if not context.device:
            log.error(
                'No devices connected, cannot invoke %s.%s' %
...
def no_emulator(f):
    ...
    def wrapper(*a, **kw):
        ...
        if c and c.port == 'emulator':
            log.error(
                "Cannot invoke %s.%s on an emulator." %
                (f.__module__, f.__name__))
        return f(*a, **kw)
    return wrapper
...
class AdbDevice(Device):
    ...
    def __init__(self, serial, type, port=None, product='unknown',
                 model='unknown', device='unknown', features=None, **kw):
        self.serial = serial
        self.type = type
        self.port = port
...
def push(local_path, remote_path):
    ...
    .. doctest::
       :skipif: skip_android
        ...
        ...
        PwntoolsException: Could not stat '/does/not/exist'
    """
...
def mkdir(path):
    ...
    .. doctest::
       :skipif: skip_android
        ...
        Traceback (most recent call last):
        ...
        PwntoolsException: mkdir failed for /init, File exists
    """
...
def unlink(path, recursive=False):
    ...
    .. doctest::
       :skipif: skip_android
        ...
        Traceback (most recent call last):
        ...
        PwntoolsException: Could not unlink '/does/not/exist': Does not exist
        ...
        Traceback (most recent call last):
        ...
        PwntoolsException: Cannot delete non-empty directory '/data/local/tmp/unlink-test' without recursive=True
...
def process(argv, *a, **kw):
    ...
    See :class:`pwntools.tubes.process.process` documentation for more info.

    Returns:
        A :class:`pwntools.tubes.process.process` tube.
...
def logcat(stream=False):
    ...
    Returns:
        ...
        Otherwise, it returns a :class:`pwntools.tubes.tube.tube` connected to the log output.
    """
...
def unlock_bootloader():
    ...
    with w:
        ...
        if getprop('ro.boot.oem_unlock_support') == '0':
            log.error(
                "Bootloader cannot be unlocked: ro.boot.oem_unlock_support=0")
...
def walk(top, topdown=True):
    join = os.path.join
    def isdir(x): return stat.S_ISDIR(x['mode'])
    client = AdbClient()
...
class Partitions(object):
    ...
    def by_name_dir(self):
        ...
        except (StopIteration, PwntoolsException):
            return '/dev/block'
...
def install(apk, *arguments):
    ...
    with log.progress("Installing APK {}".format(basename)) as p:
        with context.quiet:
            ...
            p.status('Installing')
            result = process(
                ['pm', 'install-create', target_path] + list(arguments)).recvall()
...
def uninstall(package, *arguments):
    ...
    with log.progress("Uninstalling package {}".format(package)):
        with context.quiet:
            return process(['pm', 'uninstall', package] +
                           list(arguments)).recvall()
