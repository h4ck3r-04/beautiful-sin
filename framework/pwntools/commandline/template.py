...
printable_data_path = "pwntools/data" if 'sphinx' in sys.modules else pwntools.data.path

parser.add_argument(
    'exe',
    nargs='?',
    help='Target binary. If not given, the current directory is searched for an executable binary.')
...
parser.add_argument(
    '--libc',
    help='Path to libc binary to use. If not given, the current directory is searched for a libc binary.')
parser.add_argument('--path', help='Remote path of file on SSH server')
parser.add_argument(
    '--quiet',
    help='Less verbose template comments',
    action='store_true')
parser.add_argument(
    '--color',
    help='Print the output in color',
    choices=[
        'never',
        'always',
        'auto'],
...
parser.add_argument(
    '--no-auto',
    help='Do not automatically detect missing binaries',
    action='store_false',
    dest='auto')
...
def detect_missing_binaries(args):
  ...
  if not exe:
    if len(other_files) == 1:
      ...
      log.warning(
          "Failed to find challenge binary. There are multiple binaries in the current directory: %s",
          other_files)
...
def main(args):
  ...
  lookup = TemplateLookup(
      directories=[
          ...
          os.path.join(pwntools.data.path, 'templates')
      ],
