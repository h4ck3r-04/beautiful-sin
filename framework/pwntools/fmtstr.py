...
def merge_atoms_overlapping(atoms, sz, szmax, numbwritten, overflows):
  ...
  for idx, atom in enumerate(atoms):
    ...
    for nextidx, nextatom in enumerate(atoms[idx:], idx):
      ...
      # can be written with a format specifier (this excludes non-power-of-2
      # candidate sizes)
...
class AtomQueue(object):
  def __init__(self, numbwritten):
    self.queues = {sz: SortedList(key=lambda atom: atom.integer)
                   for sz in SPECIFIER.keys()}
...
def make_payload_dollar(data_offset, atoms, numbwritten=0,
  ...
  if no_dollars:
    ...
    # parameter index, use %c, so the number of bytes written is predictable
    fmt += "%c" * (data_offset - 1)
  ...
  for idx, atom in enumerate(atoms):
    ...
    if padding < 4 + context.bytes * no_dollars:
      fmt += "c" * padding
      # if do not padded with %{n}c  do not need to add something in data to
      # use as argument, since  we are not using a printf argument
    else:
      fmt += "%" + str(padding) + "c"
