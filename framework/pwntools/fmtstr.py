...
We can automate the exploitation of the process like so:
    ...
    >>> program = pwntools.data.elf.fmtstr.get('i386')
...
from pwntools.memleak import MemLeak
from pwntools.util.cyclic import *
from pwntools.util.fiddling import randoms
from pwntools.util.packing import *
...
class AtomWrite(object):
    ...
    def __eq__(self, other):
        if not isinstance(other, AtomWrite):
            raise TypeError(
                "comparision not supported between instances of '%s' and '%s'" %
    ...
    def __repr__(self):
        return "AtomWrite(start=%d, size=%d, integer=%#x, mask=%#x)" % (
            self.start, self.size, self.integer, self.mask)
    ...
    def compute_padding(self, counter):
        ...
        Examples:
            ...
            >>> hex(pwntools.fmtstr.AtomWrite(0x0, 0x2, 0x2345).compute_padding(0x1111))
            '0x1234'
            >>> hex(pwntools.fmtstr.AtomWrite(0x0, 0x2, 0xaa00).compute_padding(0xaabb))
            '0xff45'
            >>> hex(pwntools.fmtstr.AtomWrite(0x0, 0x2, 0xaa00, 0xff00).compute_padding(0xaabb)) # with mask
            '0x0'
        """
    ...
    def union(self, other):
        ...
        Example:
            ...
            >>> pwntools.fmtstr.AtomWrite(0x0, 0x1, 0x1, 0xff).union(pwntools.fmtstr.AtomWrite(0x1, 0x1, 0x2, 0x77))
        ...
        return AtomWrite(self.start, self.size +
                         other.size, newinteger, newmask)
    ...
    def __getitem__(self, i):
        if not isinstance(i, slice):
            if i < 0 or i >= self.size:
                raise IndexError(
                    "out of range [0, " + str(self.size) + "): " + str(i))
            i = slice(i, i + 1)
        start, stop, step = i.indices(self.size)
        if step != 1:
            raise IndexError(
                "slices with step != 1 not supported for AtomWrite")
        ...
        return AtomWrite(self.start + start, stop - start,
                         (self.integer >> shift) & clip, (self.mask >> shift) & clip)
...
def make_atoms_simple(address, data, badbytes=frozenset()):
    ...
    bad bytes, it simply returns one atom for each byte:
        ...
        >>> pwntools.fmtstr.make_atoms_simple(0x0, b"abc", set())
    ...
    RuntimeError will be raised:
        ...
        >>> pwntools.fmtstr.make_atoms_simple(0x61, b'abc', b'\x62')
        [AtomWrite(start=97, size=2, integer=0x6261, mask=0xffff), AtomWrite(start=99, size=1, integer=0x63, mask=0xff)]
        >>> pwntools.fmtstr.make_atoms_simple(0x61, b'a'*0x10, b'\x62\x63\x64\x65\x66\x67\x68')
    ...
    if any(x in badbytes for x in pack(address)):
        raise RuntimeError(
            "impossible to avoid a bad byte in starting address %x" %
            address)
    ...
    while i < len(data):
        ...
        while candidate.end < end and any(
                x in badbytes for x in pack(candidate.end)):
            candidate = candidate.union(
                AtomWrite(candidate.end, 1, data[i + candidate.size]))

        sz = min([s for s in SPECIFIER if s >= candidate.size] + [float("inf")])
        if candidate.start + sz > end:
            raise RuntimeError(
                "impossible to avoid badbytes starting after offset %d (address %#x)" %
                (i, i + address))
        i += candidate.size
        candidate = candidate.union(
            AtomWrite(
...
def find_min_hamming_in_range_step(prev, step, carry, strict):
    ...
    Examples:
        ...
        >>> pwntools.fmtstr.find_min_hamming_in_range_step(initial, (0, 0xFF, 0x1), 0, 0)
        (1, 1, 255)
        >>> pwntools.fmtstr.find_min_hamming_in_range_step(initial, (0, 1, 1), 0, 0)
        (1, 1, 255)
        >>> pwntools.fmtstr.find_min_hamming_in_range_step(initial, (0, 1, 1), 0, 1)
        (0, 0, 0)
        >>> pwntools.fmtstr.find_min_hamming_in_range_step(initial, (0, 1, 0), 0, 1)
        (1, 0, 255)
        >>> repr(pwntools.fmtstr.find_min_hamming_in_range_step(initial, (0xFF, 0x00, 0xFF), 1, 0))
        'None'
    """
    ...
    if lowbyte + carryadd <= valbyte:
        if prev[(0, val_require_strict)]:
            ...
            return prev_score + \
                1, (prev_val << 8) | valbyte, (prev_mask << 8) | 0xFF
    ...
    if prev_for_val and (
            not prev_for_other or prev_for_other[0] <= prev_for_val[0] + 1):
        return prev_for_val[0] + 1, (prev_for_val[1] <<
                                     8) | valbyte, (prev_for_val[2] << 8) | 0xFF
    if prev_for_other:
        return prev_for_other[0], (prev_for_other[1] <<
                                   8) | lowcarrybyte, (prev_for_other[2] << 8)
    return None
...
def find_min_hamming_in_range(maxbytes, lower, upper, target):
    ...
    Examples:
        ...
        >>> pp(pwntools.fmtstr.find_min_hamming_in_range(1, 0x0, 0x100, 0xaa))
        (1, '0xaa', '0xff')
        >>> pp(pwntools.fmtstr.find_min_hamming_in_range(1, 0xbb, 0x100, 0xaa))
        (0, '0xbb', '0x0')
        >>> pp(pwntools.fmtstr.find_min_hamming_in_range(1, 0xbb, 0x200, 0xaa))
        (1, '0x1aa', '0xff')
        >>> pp(pwntools.fmtstr.find_min_hamming_in_range(2, 0x0, 0x100, 0xaa))
        (2, '0xaa', '0xffff')
        >>> pp(pwntools.fmtstr.find_min_hamming_in_range(4, 0x1234, 0x10000, 0x0))
...
def merge_atoms_overlapping(atoms, sz, szmax, numbwritten, overflows):
    ...
    for idx, atom in enumerate(atoms):
        ...
        for nextidx, nextatom in enumerate(atoms[idx:], idx):
            ...
            # can be written with a format specifier (this excludes
            # non-power-of-2 candidate sizes)
            ...
            if approxed.size > sz:
                score, v, m = find_min_hamming_in_range(
                    approxed.size, numbwritten_here, maxwritten, approxed.integer)
        ...
        # also update the numbwritten_at for all the indices covered by the
        ...
        for i, iatom in enumerate(atoms[idx:nextidx + 1], idx):
            ...
            # mark it as done
            if not (iatom.mask & (
                    ~best_candidate[offset:offset + shift].mask)):
                done[i] = True
            else:
                # numbwritten_at is only relevant for atoms that aren't done yet,
                # so update it only in that case (done atoms are never
                # processed again)
...
class AtomQueue(object):
    def __init__(self, numbwritten):
        self.queues = {
            sz: SortedList(
                key=lambda atom: atom.integer) for sz in SPECIFIER.keys()}
    ...
    def pop(self):
        ...
        active_sizes = [
            sz for sz,
            p in self.positions.items() if p < len(
                self.queues[sz])]
...
def make_payload_dollar(data_offset, atoms, numbwritten=0,
                        countersize=4, no_dollars=False):
    r'''
    ...
    Examples:
        ...
        >>> pwntools.fmtstr.make_payload_dollar(1, [pwntools.fmtstr.AtomWrite(0x0, 0x1, 0xff)])
    ...
    if no_dollars:
        # since we can't dynamically offset, we have to increment manually the
        # parameter index, use %c, so the number of bytes written is
        # predictable
        fmt += "%c" * (data_offset - 1)
        # every %c write a byte, so we need to keep track of that to have the
        # right pad
    ...
    for idx, atom in enumerate(atoms):
        ...
        if countersize == 32 and counter > 2147483600:
            log.warn(
                "number of written bytes in format string close to 1 << 31. this will likely not work on glibc")
        ...
        # we also add ( context.bytes * no_dollars ) because ,
        # "%nccccccccc%n...ptr1ptr2" is more convenient than
        # %"n%8c%n...ptr1ccccccccptr2"
        if padding < 4 + context.bytes * no_dollars:
            fmt += "c" * padding
            # if do not padded with %{n}c  do not need to add something in data
            # to use as argument, since  we are not using a printf argument
        else:
            fmt += "%" + str(padding) + "c"
...
def make_atoms(writes, sz, szmax, numbwritten, overflows, strategy, badbytes):
    ...
    for address, data in normalize_writes(writes):
        ...
        if strategy == 'small':
            atoms = merge_atoms_overlapping(
                atoms, sz, szmax, numbwritten, overflows)
        elif strategy == 'fast':
...
def fmtstr_split(offset, writes, numbwritten=0, write_size='byte', write_size_max='long',
                 overflows=16, strategy="small", badbytes=frozenset(), no_dollars=False):
    """
    ...
    return make_payload_dollar(
        offset, atoms, numbwritten, no_dollars=no_dollars)


def fmtstr_payload(offset, writes, numbwritten=0, write_size='byte', write_size_max='long',
                   overflows=16, strategy="small", badbytes=frozenset(), offset_bytes=0, no_dollars=False):
    ...
    for _ in range(1000000):
        ...
        fmt, data = make_payload_dollar(
            offset + data_offset, all_atoms, numbwritten=numbwritten, no_dollars=no_dollars)
        ...
        raise RuntimeError(
            "this is a bug ... format string building did not converge")
...
class FmtStr(object):
    ...
    def __init__(self, execute_fmt, offset=None, padlen=0,
                 numbwritten=0, badbytes=frozenset()):
    ...
    def _leaker(self, addr):
        ...
        if addr & 0xfff == 0 and self.leaker._leak(
                addr + 1, 3, False) == b"ELF":
            return b"\x7f"
