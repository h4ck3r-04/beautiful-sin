...
from pwntools.util import fiddling
...
def normalize_argv_env(argv, env, log, level=2):
    ...
    if not all(isinstance(arg, (six.text_type, bytes, bytearray))
               for arg in argv):
...
def run_in_new_terminal(command, terminal=None, args=None,
                        kill_at_exit=True, preexec_fn=None):
    ...
    if not terminal:
        if context.terminal:
            ...
            for line in lines:
                ...
                if len(parts) == 3 and parts[2].startswith('MainWindow_'):
                    ...
                    with subprocess.Popen((qdbus, konsole_dbus_service, '/konsole/' + name,
                        ...
                        target_window_id = proc.communicate()[
                            0].decode().strip()
            ...
            if is_wsl and which('cmd.exe') and which(
                    'wsl.exe') and which('bash.exe'):
                terminal = 'cmd.exe'
                args = ['/c', 'start']
    ...
    if isinstance(command, six.string_types):
        ...
        # it is parsed correctly, and we do not need to account for shell
        # expansion
        script = '''
#!{executable!s}
import os
...
""".format(argv[-1])
    ...
    if terminal == 'tmux':
        ...
        if pid is None:
            log.error(
                "Could not parse PID from tmux output (%r). Start tmux first.", out)
    elif terminal == 'qdbus':
...
def dealarm_shell(tube):
    ...
    if tube.recvline().startswith('/'):
        tube.sendline(
            '''exec python -c "import signal, os; signal.alarm(0); os.execl('$SHELL','')"''')
        return tube
