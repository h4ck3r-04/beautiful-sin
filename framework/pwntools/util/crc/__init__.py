...
from pwntools.util.crc import known
...
class BitPolynom(object):
    ...
    def __init__(self, n):
        if isinstance(n, (bytes, six.text_type)):
            ...
            raise TypeError(
                "Polynomial must be called with a string or integer")
...
class Module(types.ModuleType):
    ...
    def __getattr__(self, attr):
        ...
        if attr == '__all__':
            return ['BitPolynom', 'generic_crc', 'cksum',
                    'find_crc_function'] + sorted(crcs.keys())
    ...
    @staticmethod
    def _make_crc(name, polynom, width, init, refin,
                  refout, xorout, check, extra_doc=''):
        def inner(data):
            return crc.generic_crc(data, polynom, width,
