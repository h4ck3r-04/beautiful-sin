...
def random_product(*args, **kwargs):
    ...
    if kwargs != {}:
        raise TypeError(
            'random_product() does not support argument %s' %
            kwargs.popitem())
...
def bruteforce(func, alphabet, length, method='upto',
               start=None, databag=None):
    ...
    if method == 'fixed':
        ...
        total_iterations = (len(alphabet) ** (length + 1) //
                            (len(alphabet) - 1)) - 1
...
def mbruteforce(func, alphabet, length, method='upto',
                start=None, threads=None):
    ...
    while not done:
        ...
        items_done = sum([x["items_done"]
                         for x in shareddata if x is not None])
        items_total = sum([x["items_total"]
                          for x in shareddata if x is not None])
