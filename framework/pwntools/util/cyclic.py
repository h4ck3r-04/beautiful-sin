...
from pwntools.context import context, LocalNoarchContext
from pwntools.log import getLogger
from pwntools.util import packing, iters

log = getLogger(__name__)

# Taken from https://en.wikipedia.org/wiki/De_Bruijn_sequence but changed
# to a generator
...
def cyclic(length=None, alphabet=None, n=None):
    ...
    Example:
        ...
        ...
        PwntoolsException: Can't create a pattern length=512 with len(alphabet)==3 and n==3
    ...
    if isinstance(subseq, six.integer_types):
        if subseq >= 2**(8 * n):
            ...
            notice = ("cyclic_find() expected an integer argument <= {cap:#x}, you gave {gave:#x}\n"
                ...
                cap=2**(8 * n) - 1,
                gave=subseq,
                # The number of bytes needed to represent subseq, rounded to
                # the next 4
                fits=int(
                    round(
                        float(
                            subseq.bit_length()) / 32 + 0.5) * 32) // 8,
                n=n,
            )
