...
def unbits(s, endian='big'):
    ...
    """
    if endian == 'little':
        def u(s): return packing._p8lu(int(s[::-1], 2))
    elif endian == 'big':
        def u(s): return packing._p8lu(int(s, 2))
    else:
    ...
    for c in s:
        if c in ['1', 1, True]:
            ...
            raise ValueError(
                "unbits(): cannot decode the value %r into a bit" %
                c)
...
def xor(*args, **kwargs):
    ...
    if kwargs != {}:
        raise TypeError(
            "xor() got an unexpected keyword argument '%s'" %
            kwargs.popitem()[0])
    ...
    strs = [
        packing.flat(
            ...
            word_size=8,
            sign=False,
            endianness='little') for s in args]
...
def rol(n, k, word_size=None):
    ...
    if not isinstance(word_size, six.integer_types) or word_size <= 0:
        raise ValueError(
            "rol(): 'word_size' must be a strictly positive integer")
    ...
    if isinstance(n, (bytes, six.text_type, list, tuple)):
        ...
        raise ValueError(
            "rol(): 'n' must be an integer, string, list or tuple")
...
def hexdump_iter(fd, width=16, skip=True, hexii=False, begin=0, style=None,
    ...
    while True:
        ...
        if skip and last_unique:
            ...
            lines_are_sequential = (
                cyclic and sequential_lines(
                    last_unique, chunk))
            last_unique = chunk
