...
"""
    ...
    with context.local(**kwargs):
        ...
        if not isinstance(sign, bool):
            raise ValueError(
                "pack(): sign must be either True or False (got %r)" %
                sign)
...
def unpack(data, word_size=None):
    ...
    if word_size == 'all':
        ...
        raise ValueError(
            "unpack(): word_size must be a positive integer or the string 'all'")

    byte_size = (word_size + 7) // 8

    if byte_size != len(data):
        raise ValueError(
            "unpack(): data must have length %d, since word_size was %d" %
            (byte_size, word_size))
...
def _fit(pieces, preprocessor, packer, filler, stacklevel=1):
    ...
    negative = {
        k: v for k,
        v in pieces.items() if isinstance(
            k,
            int) and k < 0}
    ...
    if negative:
        ...
        for k, v in sorted(negative.items()):
            ...
            # Recursively flatten data
            out_negative += _flat([v], preprocessor,
                                  packer, filler, stacklevel + 1)
...
def _flat(args, preprocessor, packer, filler, stacklevel=1):
    ...
    for arg in args:
        ...
        if hasattr(arg, '__flat__'):
            ...
            filler, val = _fit(
                arg, preprocessor, packer, filler, stacklevel + 1)
            ...
            raise ValueError(
                "flat(): Flat does not support values of type %s" %
                type(arg))
...
def flat(*args, **kwargs):
    ...
    from pwntools.util import cyclic
    ...
    if kwargs != {}:
        raise TypeError(
            "flat() does not support argument %r" %
            kwargs.popitem()[0])
    ...
    if isinstance(src, six.text_type):
        ...
        for i, b in enumerate(src, seek):
            ...
            if isinstance(b, bytes):
                ...
                if b > 255 or b < 0:
                    raise ValueError(
                        "dd(): Source value %d at index %d is not in range [0;255]" %
                        (b, i))
                src_ += _p8lu(b)
            else:
                raise TypeError(
                    "dd(): Unsupported `src` element type: %r" %
                    type(b))
        src = src_
