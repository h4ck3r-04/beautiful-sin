...
from pwntools.util.packing import pack, _p8lu
...
class MemLeak(object):
    ...
    Arguments:
        ...
        reraise (bool): Whether to reraise call :func:`pwntools.log.warning` in case the leaker function throws an exception.

    Example:

        >>> import pwntools
        >>> binsh = pwntools.util.misc.read('/bin/sh')
        >>> @pwntools.memleak.MemLeak
        ... def leaker(addr):
        ...
        >>> leak = pwntools.memleak.MemLeak(relative_leak, relative = True)
        >>> leak[-1:2]
        b'zAB'
    """
    ...
    def struct(self, address, struct):
        ...
        Examples:
            ...
            ... def leaker(addr):
            ...     return b"A"
            >>> e = leaker.struct(0, pwntools.elf.Elf32_Phdr)
            >>> hex(e.p_paddr)
            '0x41414141'
    ...
    def NoNulls(function):
        ...
        def null_wrapper(address, *a, **kw):
            if b'\x00' in pack(address):
                log.info(
                    'Ignoring leak request for %#x: Contains NULL bytes' %
                    address)
                return None
    ...
    def NoWhitespace(function):
        ...
        def whitespace_wrapper(address, *a, **kw):
            if set(pack(address)) & set(string.whitespace.encode()):
                log.info(
                    'Ignoring leak request for %#x: Contains whitespace' %
                    address)
                return None
            return function(address, *a, **kw)
    ...
    def NoNewlines(function):
        ...
        def whitespace_wrapper(address, *a, **kw):
            if b'\n' in pack(address):
                log.info(
                    'Ignoring leak request for %#x: Contains newlines' %
                    address)
                return None
            return function(address, *a, **kw)
