...
# ./pwntools/data/includes/darwin/aarch64.h
# ./pwntools/constants/darwin/aarch64.py
...
regex = re.compile(
    r'^#define\s+([a-zA-Z0-9_-]+)\s+([0-9]+|0x[0-9a-fA-F]+)(?:\s|$)',
    re.DOTALL)
...
for file in sdk_path.iterdir():
    ...
    for line in file.read_text(errors='ignore').split('\n'):
        ...
        if value.startswith('0') and not value.startswith(
                '0x') and len(value) > 1:
            value = '0o' + value
...
outbuf1_aarch64 = ''
outbuf1_aarch64 += "from pwntools.constants.constant import Constant\n"

outbuf1_amd64 = ''
outbuf1_amd64 += "from pwntools.constants.constant import Constant\n"
...
pp = project_path
(pp / Path('./pwntools/constants/darwin/aarch64.py')
 ).write_bytes(outbuf1_aarch64.encode())
(pp / Path('./pwntools/data/includes/darwin/aarch64.h')
 ).write_bytes(outbuf2_aarch64.encode())

(pp / Path('./pwntools/constants/darwin/amd64.py')
 ).write_bytes(outbuf1_amd64.encode())
(pp / Path('./pwntools/data/includes/darwin/amd64.h')
 ).write_bytes(outbuf2_amd64.encode())
