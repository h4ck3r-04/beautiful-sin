...
them in the environment prefixed by ``PWNTOOLS_``.

The easiest example is to enable more verbose debugging.  Just set ``DEBUG``.

.. code-block:: bash

  $ PWNTOOLS_DEBUG=1 python exploit.py
  $ python exploit.py DEBUG

These arguments are automatically extracted, regardless of their name, and
exposed via :mod:`pwntools.args.args`, which is exposed as the global variable
