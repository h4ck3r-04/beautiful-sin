...
from pwntools.context import context, LocalContext
from pwntools.exception import PwntoolsException
from pwntools.log import Logger
from pwntools.log import getLogger
from pwntools.term import text
from pwntools.timeout import Timeout
from pwntools.tubes.sock import sock
from pwntools.util import hashes
from pwntools.util import misc
from pwntools.util import packing
from pwntools.util import safeeval
from pwntools.util.sh_string import sh_string
...
class ssh_channel(sock):
    ...
    def __init__(self, parent, process=None, tty=False,
                 cwd=None, env=None, raw=True, *args, **kwargs):
        ...
        if process and env:
            for name, value in env.items():
                ...
                export = b'export %s=%s;' % (nameb, sh_string(
                    packing._need_bytes(value, 2, 0x80)))
        ...
        with self.waitfor(msg) as h:
            ...
            try:
                ...
                if e.args == (1, 'Administratively prohibited'):
                    self.error(
                        "Too many sessions open! Use ssh_channel.close() or 'with'!")
                raise e
    ...
    def interactive(self, prompt=term.text.bold_red('$') + ' '):
        """interactive(prompt = pwntools.term.text.bold_red('$') + ' ')

        If not in TTY-mode, this does exactly the same as
        meth:`pwntools.tubes.tube.tube.interactive`, otherwise
        ...
        once the :mod:`pwntools.term` is more mature.
        """

        # If we are only executing a regular old shell, we need to handle
        # control codes (specifically Ctrl+C).
        #
        # Otherwise, we can just punt to the default implementation of
        # interactive()
...
class ssh_process(ssh_channel):
    ...
    @property
    def elf(self):
        """elf() -> pwntools.elf.elf.ELF

        Returns an ELF file for the executable that launched the process.
        """
        import pwntools.elf.elf
        ...
        for lib in libs:
            ...
            if lib.endswith(self.executable):
                return pwntools.elf.elf.ELF(lib)

    @property
    def corefile(self):
        import pwntools.elf.corefile

        finder = pwntools.elf.corefile.CorefileFinder(self)
        if not finder.core_path:
            self.error("Could not find core file for pid %i" % self.pid)

        return pwntools.elf.corefile.Corefile(finder.core_path)
    ...
    def getenv(self, variable, **kwargs):
        ...
        try:
            with context.quiet:
                python = self.parent.which('python2.7') or self.parent.which(
                    'python3') or self.parent.which('python')
            ...
            self.exception(
                "Could not look up environment variable %r" %
                variable)
...
class ssh_connecter(sock):
    def __init__(self, parent, host, port, *a, **kw):
        ...
        msg = 'Connecting to %s:%d via SSH to %s' % (
            self.rhost, self.rport, self.host)
        with self.waitfor(msg) as h:
            try:
                self.sock = parent.transport.open_channel(
            ...
            try:
                # Iterate all layers of proxying to get to base-level Socket
                # object
    ...
    def _close_msg(self):
        self.info(
            "Closed remote connection to %s:%d via SSH connection to %s" %
            (self.rhost, self.rport, self.host))
...
class ssh_listener(sock):
    def __init__(self, parent, bind_address, port, *a, **kw):
        ...
        try:
            self.port = parent.transport.request_port_forward(
                bind_address, port)
...
class ssh(Timeout, Logger):
    ...
    def __init__(self, user=None, host=None, port=22, password=None, key=None,
        ...
        self._cachedir = os.path.join(
            tempfile.gettempdir(), 'pwntools-ssh-cache')
        self.cache = cache
        self.raw = raw
        ...
        try:
            ...
            self.debug(
                "An error occurred while parsing ~/.ssh/config:\n%s" %
                e)
        ...
        with self.waitfor(msg) as h:
            ...
            if has_proxy:
                if 'ProxyCommand' not in dir(paramiko):
                    self.error(
                        'This version of paramiko does not support proxies.')

                if proxy_sock and proxy_command:
                    self.error(
                        'Cannot have both a proxy command and a proxy sock')
            ...
            try:
                ...
                if user and auth_none and str(
                        e) == "No authentication methods available":
        ...
        try:
            ...
            self.warn_once(
                "Couldn't check security settings on %r" %
                self.host)

    def __repr__(self):
        return "{}(user={!r}, host={!r})".format(
            self.__class__.__name__, self.user, self.host)
    ...
    def shell(self, shell=None, tty=True, timeout=Timeout.default):
        ...
        Returns:
            Return a :class:`pwntools.tubes.ssh.ssh_channel` object.
    ...
    def process(self, argv=None, executable=None, tty=True, cwd=None, env=None, ignore_environ=None, timeout=Timeout.default, run=True,
        ...
        as pwntools.tubes.process.process.
        ...
        Arguments:
            ...
            aslr(bool):
                See :class:`pwntools.tubes.process.process` for more information.
            setuid(bool):
                See :class:`pwntools.tubes.process.process` for more information.
            shell(bool):
        ...
        Examples:
            ...
            ...
            PwntoolsException: preexec_fn must be a function

            >>> s.process('false', preexec_fn=lambda: 1234)
            Traceback (most recent call last):
            ...
            PwntoolsException: preexec_fn cannot be a lambda
        ...
        if not run:
            with context.local(log_level='error'):
                tmpfile = self.mktemp('-t', 'pwntools-execve-XXXXXXXXXX')
        ...
        with self.progress(msg) as h:
            ...
            script = 'echo PWNTOOLS; for py in python3 python2.7 python2 python; do test -x "$(command -v $py 2>&1)" && echo $py && exec $py -c %s check; done; echo 2' % sh_string(
                script)
            with context.quiet:
                python = ssh_process(
                    self,
            ...
            try:
                # Magic flag so that any sh/bash initialization errors are
                # swallowed
                python.recvline_contains(b'PWNTOOLS')
                try:
                ...
                # Status flag from the Python script
            ...
            if result == 0:
                self.error(
                    "%r does not exist or is not executable" %
                    executable)
            elif result == 3:
                self.error("%r" % error_message)
            elif result == 2:
                self.error(
                    "python is not installed on the remote system %r" %
                    self.host)
            elif result != 1:
        ...
        if not aslr and setuid and (
                python.uid != python.suid or python.gid != python.sgid):
    ...
    def which(self, program):
        ...
        result = self.system(
            b'export PATH=$PATH:$PWD; command -v ' +
            program).recvall().strip()
    ...
    def system(self, process, tty=True, cwd=None,
               env=None, timeout=None, raw=True, wd=None):
        ...
        if wd is not None:
            self.warning_once(
                "The 'wd' argument to ssh.system() is deprecated.  Use 'cwd' instead.")
            if cwd is None:
                cwd = wd
        if cwd is None:
            cwd = self.cwd
        ...
        return ssh_channel(self, process, tty, cwd, env,
                           timeout=timeout, level=self.level, raw=raw)
...
''' % variable
        ...
        with context.local(log_level='error'):
            python = self.which('python') or self.which(
                'python2.7') or self.which('python3')

            if not python:
                self.error("Python is not installed on the remote system.")

            io = self.process(['', '-c', script.strip()],
                              executable=python, **kwargs)
            result = io.recvall()
    ...
    def run_to_end(self, process, tty=False, cwd=None, env=None, wd=None):
        ...
        if wd is not None:
            self.warning_once(
                "The 'wd' argument to ssh.run_to_end() is deprecated.  Use 'cwd' instead.")
            if cwd is None:
                cwd = wd
    ...
    def connect_remote(self, host, port, timeout=Timeout.default):
        ...
        Returns a :class:`pwntools.tubes.ssh.ssh_connecter` object.
    ...
    def listen_remote(self, port=0, bind_address='', timeout=Timeout.default):
        ...
        Returns a :class:`pwntools.tubes.ssh.ssh_listener` object.
        ...
        return ssh_listener(self, bind_address, port,
                            timeout, level=self.level)
    ...
    def __getattr__(self, attr):
        ...
        def runner(*args):
            if len(args) == 1 and isinstance(args[0], (list, tuple)):
                ...
                command = b' '.join(
                    packing._need_bytes(
                        arg, min_wrong=0x80) for arg in command)
    ...
    def download_dir(self, remote=None, local=None, ignore_failed_read=False):
        ...
        if self.sftp:
            ...
            with context.local(log_level='error'):
                remote = self.system(
                    b'readlink -f ' +
                    sh_string(remote)).recvall().strip()
    ...
    def upload_dir(self, local, remote=None):
        ...
        with self.waitfor(msg):
            ...
            with context.local(log_level='error'):
                ...
                untar = self.run(
                    b'cd %s && tar -xzf %s' %
                    (sh_string(remote), sh_string(remote_tar)))
                message = untar.recvrepeat(2)

                if untar.wait() != 0:
                    self.error(
                        "Could not untar %r on the remote end\n%s" %
                        (remote_tar, message))
    ...
    def interactive(self, shell=None):
        ...
        :class:`pwntools.tubes.ssh.ssh_channel` object and calling
        :meth:`pwntools.tubes.ssh.ssh_channel.interactive` on it."""
    ...
    def set_working_directory(self, wd=None, symlink=False):
        ...
        if symlink and not isinstance(
                symlink, (six.binary_type, six.text_type)):
        ...
        if not wd:
            wd, status = self.run_to_end(
                'x=$(mktemp -d) && cd $x && chmod +x . && echo $PWD', cwd='.')
            wd = wd.strip()

            if status:
                self.error(
                    "Could not generate a temporary directory (%i)\n%s" %
                    (status, wd))
    ...
    def write(self, path, data):
        """Wrapper around upload_data to match :func:`pwntools.util.misc.write`"""
        data = packing._need_bytes(data)
        return self.upload_data(data, path)

    def read(self, path):
        """Wrapper around download_data to match :func:`pwntools.util.misc.read`"""
    ...
    def distro(self):
        ...
        try:
            self._init_remote_platform_info()
            return (self._platform_info['distro'],
                    self._platform_info['distro_ver'])
        except Exception:
    ...
    def aslr_ulimit(self):
        ...
        with context.local(arch=arch, bits=32, os=self.os, aslr=True):
            with context.quiet:
                try:
                    sc = pwntools.shellcraft.cat('/proc/self/maps') \
                        + pwntools.shellcraft.exit(0)

                    elf = pwntools.elf.elf.ELF.from_assembly(sc, shared=True)
                except Exception:
                ...
                maps = self.process(
                    ['./aslr-test'], preexec_fn=preexec).recvall()
    ...
    def _cpuinfo(self):
        if self._cpuinfo_cache is None:
            with context.quiet:
                try:
                    self._cpuinfo_cache = self.download_data(
                        '/proc/cpuinfo', fingerprint=False)
                except PwntoolsException:
    ...
    def checksec(self, banner=True):
        ...
        if self.aslr_ulimit:
            res += ["Note:".ljust(10) +
                    red("Susceptible to ASLR ulimit trick (CVE-2016-3672)")]
