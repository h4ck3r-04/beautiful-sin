...
from pwntools.util.packing import _decode
from pwntools.util.misc import normalize_argv_env
from pwntools.util.misc import which
from pwntools.util.misc import parse_ldd_output
from pwntools.util.hashes import sha256file
from pwntools.tubes.tube import tube
...
signal_names = {-v: k for k, v in signal.__dict__.items()
                if k.startswith('SIG')}
...
class process(tube):
    ...
    Arguments:
        ...
        stderr(int):
            ...
            although the :class:`pwntools.tubes.tube.tube` wrapper will not be able to read this data.
        close_fds(bool):
    ...
    def __init__(self, argv=None,
        ...
        self.env = {} if ignore_environ else dict(
            getattr(os, "environb", os.environ))
        ...
        if IS_WINDOWS:
            ...
            if self.proc.stdout:
                ...
                self._read_thread = threading.Thread(
                    target=_read_in_thread, args=(
                        self._read_queue, self.proc.stdout))
    ...
    def cwd(self):
        ...
        """
        try:
            from pwntools.util.proc import cwd
    ...
    def can_recv_raw(self, timeout):
        ...
        try:
            if timeout is None:
                return select.select([self.proc.stdout], [], []) == (
                    [self.proc.stdout], [], [])

            return select.select([self.proc.stdout], [], [], timeout) == (
                [self.proc.stdout], [], [])
        except ValueError:
            # Not sure why this isn't caught when testing self.proc.stdout.closed,
            # but it's not.
            #
            #   File "/home/user/pwntools/pwntools/tubes/process.py", line 112, in can_recv_raw
    ...
    def shutdown_raw(self, direction):
        ...
        if all(fp is None or fp.closed for fp in [
    ...
    def libs(self):
        ...
        space.
        """
        from pwntools.util.proc import memory_maps
        ...
        if not maps_raw:
            ...
            with context.quiet:
                return pwntools.elf.elf.ELF(self.executable).maps
    ...
    def elf(self):
        ...
        """
        import pwntools.elf.elf
        return pwntools.elf.elf.ELF(self.executable)

    @property
    def corefile(self):
        """corefile() -> pwntools.elf.elf.Core
        ...
        try:
            if self.poll() is None:
                corefile = pwntools.gdb.corefile(self)
                if corefile is None:
                    self.error(
                        "Could not create corefile with GDB for %s",
                        self.executable)
                return corefile
            ...
            self._corefile = pwntools.elf.corefile.Corefile(finder.core_path)
        except AttributeError as e:
            # AttributeError would route through __getattr__, losing original
            # message
