...
from pwntools.tubes.buffer import Buffer
...
class tube(Timeout, Logger):
    ...
    def recvlinesS(self, numlines=2**20, keepends=False, timeout=default):
        ...
        Examples:
            ...
            ['Foo', 'Bar', 'Baz']
        """
        return [packing._decode(x) for x in self.recvlines(
            numlines, keepends, timeout)]
    ...
    def recvlinesb(self, numlines=2**20, keepends=False, timeout=default):
        ...
        """
        return [bytearray(x)
                for x in self.recvlines(numlines, keepends, timeout)]
    ...
    def recvline(self, keepends=True, timeout=default):
        ...
        This behavior can be changed by setting :meth:`pwntools.context.ContextType.throw_eof_on_incomplete_line`.
        ...
        try:
            return self.recvuntil(
                self.newline, drop=not keepends, timeout=timeout)
        except EOFError:
    ...
    def recvline_endswith(self, delims, keepends=False, timeout=default):
        ...
        delims = tuple(
            packing._need_bytes(delim) +
            self.newline for delim in delims)
    ...
    def recvline_regex(self, regex, exact=False,
                       keepends=False, timeout=default):
    ...
    def interactive(self, prompt=term.text.bold_red('$') + ' '):
        ...
        is much more usable, since we are using :mod:`pwntools.term` to print a
        floating prompt.

        Thus it only works while in :data:`pwntools.term.term_mode`.
        """
        ...
        from pwntools.args import term_mode
        try:
    ...
    def clean(self, timeout=0.05):
        ...
        :meth:`pwntools.tubes.tube.tube.recv` with a low timeout until it fails.
    ...
    def clean_and_log(self, timeout=0.05):
        ...
        Works exactly as :meth:`pwntools.tubes.tube.tube.clean`, but logs received
        data with :meth:`pwntools.self.info`.
    ...
    def shutdown(self, direction="send"):
        ...
        try:
            ...
            raise KeyError(
                'direction must be in %r' %
                sorted(
                    self.shutdown_directions))
        else:
    ...
    def unpack(self,
               *a,
               **kw): return packing.unpack(self.recvn(context.bytes),
                                            *a,
                                            **kw)
    ...
    def make_wrapper(func):
        ...
        wrapperb.__doc__ = 'Same as :meth:`{func.__name__}`, but returns a bytearray'.format(
            func=func)
        ...
        wrapperS.__doc__ = 'Same as :meth:`{func.__name__}`, but returns a str, ' \
                           ...
                           '(note that the binary versions are way faster)'.format(
                               func=func)
