...
------------------------
  ...
  Architecture, endianness, and word size are selected by using :mod:`pwntools.context`.
...
------------------------
  ...
  Additionally, you can use constants as defined in the :mod:`pwntools.constants`
  module.
...
def dpkg_search_for_binutils(arch, util):
  ...
  ::
    ...
    >>> pwntools.asm.dpkg_search_for_binutils('aarch64', 'as')
  ...
  # /usr/x86_64-linux-gnu/arm-linux-gnu/include/dis-asm.h
  packages = []

  try:
    filename = 'bin/%s*linux*-%s' % (arch, util)
    output = subprocess.check_output(
        ['dpkg', '-S', filename], universal_newlines=True)
...
def print_binutils_instructions(util, context):
  ...
  Doctest:
    ...
    >>> pwntools.asm.print_binutils_instructions('as', context)
    Traceback (most recent call last):
    ...
    PwntoolsException: Could not find 'as' installed for ContextType(arch = 'amd64', bits = 64, endian = 'little')
  ...
  log.error(
      """ Could not find %(util)r installed for %(context)s Try installing binutils for this architecture: %(instructions)s """.strip() %
      locals())
...
def check_binutils_version(util):
  ...
  result = subprocess.check_output([util,
                                    '--version',
                                    '/dev/null'],
...
def which_binutils(util, check_version=False):
  ...
  Examples:
    ...
    >>> import platform
    >>> which_binutils = pwntools.asm.which_binutils
...
def _include_header():
  ...
  if not include or not path.exists(path.join(_incdir, include)):
    log.warn_once(
        "Could not find system include headers for %s-%s" %
        (arch, os))
    return '\n'
...
def disasm(data, vma=0, byte=True, offset=True, instructions=True):
  ...
  # Note: those patterns are also used in pwntools/commandline/disasm.py
