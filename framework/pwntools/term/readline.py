...
from pwntools.term import keyconsts as kc
from pwntools.term import keymap as km
from pwntools.term import term
...
def readline(_size=-1, prompt='', float=True, priority=10):
    ...
    from pwntools.term import term_mode
    if not term_mode:
        six.print_(prompt, end='', flush=True)
        return getattr(sys.stdin, 'buffer', sys.stdin).readline(
            _size).rstrip(b'\n')
    show_suggestions = False
    eof = False
    if prompt:
...
def eval_input(prompt='', float=True):
    ...
    return safeeval.const(
        readline(-1, prompt, float).rstrip(os.linesep.encode()))
    ...
    class Wrapper:
        ...
        def readline(self, size=None):
            ...
            if isinstance(self._fd, io.TextIOWrapper):
                r = r.decode(
                    encoding=self._fd.encoding,
                    errors=self._fd.errors)
            return r
