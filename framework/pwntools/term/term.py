...
tmap = {c: '\\x{:02x}'.format(c) for c in set(
    range(0x20)) - {0x09, 0x0a, 0x0d, 0x1b} | {0x7f}}
...
class Cell(object):
  ...
  def update_locked(self, erase_line=False):
    ...
    for cell in cells.iter_after(self):
      ...
      if pos[1] < cell.pos[1]:
        # the cell moved left, it must be same line as self; erase if not yet
        # erased
  ...
  def __repr__(self):
    return '{}({!r}, float={}, pos={})'.format(
        self.__class__.__name__, self.value, self.float, self.pos)
...
def output(s='', float=False, priority=10,
           frozen=False, indent=0, before=None):
  with wlock:
    if before:
      float = before.float
