...
- It is not possible to tell whether ``gdb.execute('continue')`` will be
  ...
  synchronous :func:`pwntools.gdb.Gdb.continue_and_wait` or the explicitly
  asynchronous :func:`pwntools.gdb.Gdb.continue_nowait` instead.
...
``NOPTRACE`` argument appended, or with ``PWNTOOLS_NOPTRACE=1`` in the environment.
...
from pwntools import atexit
from pwntools import elf
from pwntools import qemu
from pwntools import tubes
from pwntools.asm import _bfdname
from pwntools.asm import make_elf
from pwntools.asm import make_elf_from_assembly
...
from pwntools.util import proc
...
def debug_assembly(asm, gdbscript=None, vma=None, api=False):
    ...
    Arguments:
        ...
        \**kwargs: Override any :obj:`pwntools.context.context` values.
...
def _execve_script(argv, executable, env, ssh):
    ...
    script = misc._create_execve_script(
        argv=argv, executable=executable, env=env, log=log)
    script = script.strip()
    # Create a temporary file to hold the script
    tmp = tempfile.NamedTemporaryFile(
        mode="w+t",
        prefix='pwntools-execve-',
        suffix='.py',
        delete=False)
    tmp.write(script)
...
def _gdbserver_args(pid=None, path=None, args=None,
                    which=None, env=None, python_wrapper_script=None):
...
def _gdbserver_port(gdbserver, ssh):
    ...
    try:
        ...
        log.error(
            'gdbserver did not output its pid (maybe chmod +x?): %r',
            process_created)
...
@LocalContext
def debug(args, gdbscript=None, exe=None, ssh=None,
          env=None, sysroot=None, api=False, **kwargs):
    r"""
    ...
    Using SSH:
        ...
        >>> io.close()
    """
    if isinstance(args, six.integer_types +
    ...
    if ssh or context.native or (context.os == 'android'):
        if len(args) > 0 and which(packing._decode(
                args[0])) == packing._decode(exe):
    ...
    tmp = attach((host, port), exe=exe, gdbscript=gdbscript,
                 ssh=ssh, sysroot=sysroot, api=api)
    if api:
        _, gdb = tmp
        gdbserver.gdb = gdb
...
class Breakpoint:
    ...
    def __init__(self, conn, *args, **kwargs):
        ...
        Use ``pwntools.gdb.Gdb.Breakpoint`` instead.
        """
...
class FinishBreakpoint(Breakpoint):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Use ``pwntools.gdb.Gdb.FinishBreakpoint`` instead.
        """
...
@LocalContext
def attach(target, gdbscript='', exe=None, gdb_args=None,
           ssh=None, sysroot=None, api=False):
    r"""
    ...
    if isinstance(target, six.integer_types):
        ...
        if proc.exe(pid).endswith(
                '/socat') and time.sleep(0.1) and proc.children(pid):
    ...
    if gdbscript:
        with tempfile.NamedTemporaryFile(prefix='pwntools-gdbscript-', suffix='.gdb',
...
def find_module_addresses(binary, ssh=None, ulimit=False):
    ...
    Arguments:
        ...
        ssh(pwntools.tubes.tube): SSH connection through which to load the libraries.
            If left as :const:`None`, will use a :class:`pwntools.tubes.process.process`.
        ulimit(bool): Set to :const:`True` to run "ulimit -s unlimited" before GDB.

    Returns:
        A list of pwntools.elf.ELF objects, with correct base addresses.
    ...
    #
    libs = {}
    # pipe through cat to disable colored output on GDB 9+
    cmd = "gdb -q -nh --args %s | cat" % (binary)
