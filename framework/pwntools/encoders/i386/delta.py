...
from pwntools.asm import asm
from pwntools.asm import disasm
from pwntools.context import context
from pwntools.encoders.encoder import Encoder
from pwntools.util.fiddling import hexdump
...
class i386DeltaEncoder(Encoder):
  ...
  Example:
      ...
      >>> sc = pwntools.encoders.i386.delta.encode(b'\xcc', b'\x00\xcc')
      >>> e  = ELF.from_bytes(sc)
      >>> e.process().poll(True)
      -5
  """
  ...
  def __call__(self, raw_bytes, avoid, pcreg=''):
    ...
    def not_bad(x): return six.int2byte(x) not in avoid
    def not_bad_or_term(x): return not_bad(x) and x != self.terminator
