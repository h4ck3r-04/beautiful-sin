...
class builder:
    ...
    def DecoderLoopBuilder(self, icache_flush):
        ...
        dec_loop += ARM_Instructions.dpimm(SUB,
                                           PL, 0, s, 4, chr(dist + 0x04 + offset))
    ...
    def DecoderBuilder(self, input, icache_flush):
        ...
        # trucate the last instruction, which increments raddr by 1, from the
        # output*/
        output = output[:-4]
        self.size -= 4
        ...
        output += ARM_Instructions.dpshiftreg(SUB,
                                              0, 4, self.i, self.i, LSR, self.i)
        # SUBPL r6, ri, ri LSR ri*/
        output += ARM_Instructions.dpshiftreg(SUB,
                                              0, 6, self.i, self.i, LSR, self.i)
        ...
        if icache_flush:
            ...
            # Store 4 0x00*/
            # STRPLB random_funcs.randel(arr6), [!rm, -(r5 ROR
            # #random_funcs.randel(arr5))]*/
            output += ARM_Instructions.sbyteposti(
                random_funcs.randel(arr6), m, 5, random_funcs.randel(arr5))
            output += ARM_Instructions.sbyteposti(
                random_funcs.randel(arr6), m, 5, random_funcs.randel(arr5))
            output += ARM_Instructions.sbyteposti(
                random_funcs.randel(arr6), m, 5, random_funcs.randel(arr5))
            ...
            output += ARM_Instructions.sbyteposti(5,
                                                  m, 5, random_funcs.randel(arr5))
            output += ARM_Instructions.sbyteposti(5,
                                                  m, 5, random_funcs.randel(arr5))
            output += ARM_Instructions.sbyteposti(5,
                                                  m, 5, random_funcs.randel(arr5))
    ...
    def algo1(self, input, begin_inp, iter):
        ...
        for p in range(begin_inp, begin_inp + iter):
            ...
            if alphanum_byte.alphanumeric_check(y):
                ...
                output += ARM_Instructions.dpshiftreg(
                    SUB, 0, self.addr, self.addr, self.j, ROR, self.k)
                self.size += 4
                continue
            if y >= 0x80:
                if alphanum_byte.alphanumeric_check(~y):
                    # EORPLS rk, rj, #~y*/
                    output += ARM_Instructions.dpimm(EOR,
                                                     PL, 1, self.k, self.j, ~y)
                    # STRMIB rk, [raddr, #(-offset)]*/
                    output += ARM_Instructions.lsbyte(STR,
                                                      MI, self.k, self.addr, offset)
                    # SUBMIS rk, ri, #x*/
                    output += ARM_Instructions.dpimm(SUB,
                                                     MI, 1, self.k, self.i, self.x)
                    # SUBPL raddr, raddr, rj ROR rk*/
                    output += ARM_Instructions.dpshiftreg(
                        SUB, 0, self.addr, self.addr, self.j, ROR, self.k)
            ...
            if self.x > y:
                ...
                if alphanum_byte.alphanumeric_check(z1):
                    # SUBPL rk, ri, #z*/
                    output += ARM_Instructions.dpimm(SUB,
                                                     PL, 0, self.k, self.i, z1)
                    # STRPLB rk, [raddr, #(-offset)]*/
                    output += ARM_Instructions.lsbyte(STR,
                                                      PL, self.k, self.addr, offset)
                    # SUBPL raddr, raddr, rj ROR rk*/
                    output += ARM_Instructions.dpshiftreg(
                        SUB, 0, self.addr, self.addr, self.j, ROR, self.k)
            ...
            if alphanum_byte.alphanumeric_check(z2):
                # RSBPL rk, ri, #z*/
                output += ARM_Instructions.dpimm(RSB,
                                                 PL, 0, self.k, self.i, z2)
                # STRPLB rk, [raddr, #(-offset)]*/
                output += ARM_Instructions.lsbyte(STR,
                                                  PL, self.k, self.addr, offset)
                # SUBPL raddr, raddr, rj ROR rk*/
    ...
    def buildInit(self, input):
        ...
        while p <= ((total - 8) // 4):
            ...
            if p == topv or p == topw:
                ...
                output += ARM_Instructions.dpimm(op,
                                                 cond,
                                                 s,
                                                 d,
                                                 n,
                                                 alphanum_byte.alphanumeric_get_byte())
            p += 1
