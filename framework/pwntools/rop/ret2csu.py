...
def ret2csu(rop, elf, edi, rsi, rdx, rbx, rbp, r12, r13, r14, r15, call=None):
    ...
    if '__libc_csu_init' not in elf.symbols:
        textaddr = elf.offset_to_vaddr(
            elf.get_section_by_name('.text').header.sh_offset)
        entry = elf.entry
        ...
        for insn in md.disasm(data, entry):
            if insn.mnemonic == mnemonic:
                ...
                log.error(
                    "This binary is compiled for glibc 2.34+ and does not have __libc_csu_init")
    ...
    if not elf.pie and not call:
        ...
        log.error(
            "No non-PIE binaries in [elfs], 'call' parameter is required")

    csu_function = elf.read(
        elf.sym['__libc_csu_init'],
        elf.sym['__libc_csu_fini'] -
        elf.sym['__libc_csu_init'])
    ...
    # Account for this when the binary was linked against a glibc that was
