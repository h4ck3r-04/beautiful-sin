...
class Ret2dlresolvePayload(object):
  ...
  def _build_structures(self):
    ...
    if self.elf.pie:
      rel = ElfRel(r_offset=self.data_addr -
                   (self.elf.load_addr +
                    self.elf_load_address_fixup), r_info=(index << ELF_R_SYM_SHIFT) +
                   rel_type)
  ...
  def _build_args(self):
    # The second part of the payload will include strings and pointers needed
    # for ROP.
    queue = Queue()
