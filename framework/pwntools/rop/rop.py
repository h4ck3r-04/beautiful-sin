...
from pwntools.rop import srop
from . import ret2dlresolve
from pwntools.rop.call import AppendedArgument
from pwntools.rop.call import Call
from pwntools.rop.call import CurrentStackPointer
from pwntools.rop.call import NextGadgetAddress
from pwntools.rop.call import StackAdjustment
from pwntools.rop.call import Unresolved
from pwntools.rop.gadgets import Gadget
from pwntools.util import lists
from pwntools.util import packing
from pwntools.util.cyclic import cyclic
from pwntools.util.packing import pack
from pwntools.util.misc import python_2_bytes_compatible
...
class ROP(object):
    ...
    def setRegisters(self, registers):
        ...
        for gadget in self.gadgets.values():
            ...
            # if both gadgets require same stack space, choose the one with
            # less instructions
            if (old is gadget) \
    ...
    def generatePadding(self, offset, count):
        ...
        alphabet = b''.join(packing.p8(c) for c in bytearray(
            string.ascii_lowercase.encode()) if c not in self._badchars)
    ...
    def build(self, base=None, description=None):
        ...
        for idx, slot in iterable:
            ...
            # broken down into pointer-width blobs.
                ...
                registers = [slot.registers[i]
                             for i in sorted(slot.registers.keys())]
                ...
                nextGadgetAddr = stack.next + \
                    (context.bytes * len(stackArguments))
                ...
                if slot.abi.returns:
                    ...
                    if len(stackArguments) > 0:
                        if remaining and (
                                remaining > 1 or Call.is_flat(chain[-1])):
                            ...
                            for pad in range(
                                    fix_bytes, adjust.move, context.bytes):
                                stackArguments.append(Padding())

                        # We could not find a proper "adjust" gadget, but also
                        # didn't need one.
                        elif remaining:
        ...
        for i, slot in enumerate(stack):
            if isinstance(slot, six.integer_types):
                ...
                stack[i] = self.generatePadding(
                    i * context.bytes, context.bytes)
            ...
            # Also, it may work in pwntools.util.packing.flat()
            else:
                pass
    ...
    def _srop_call(self, resolvable, arguments):
        ...
        for instruction in syscall_instructions:
            ...
            log.error(
                "Could not find any instructions in %r" %
    ...
    def __load(self):
        ...
        # False
        #
        def valid(insn): return any(map(lambda pattern: pattern.match(
            insn), [pop, add, ret, leave, int80, syscall, sysenter]))
    ...
    def ret2csu(self, edi=Padding('edi'), rsi=Padding('rsi'),
        ...
        Test:
            ...
            >>> context.clear(binary=pwntools.data.elf.ret2dlresolve.get("amd64"))
