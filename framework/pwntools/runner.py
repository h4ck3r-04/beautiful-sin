...
from pwntools.context import LocalContext, context
...
def run_assembly(assembly):
  ...
  Returns:
      ...
      A :class:`pwntools.tubes.process.process` tube to interact with the process.
  ...
  if context.os == 'darwin':
    ...
    from pwntools.asm import make_macho_from_assembly
  ...
  if context.os == 'darwin':
    if sys.platform != 'darwin':
      raise ValueError('Running Mach-O only supported on Darwin machines. Please use:\n'
                       '- https://github.com/MatthewCroughan/NixThePlanet\n'
                       '- https://github.com/sickcodes/Docker-OSX')
    from pwntools.asm import make_macho
