...
from pwntools.args import args
...
from pwntools.version import __version__
...
def check_automatically():
    ...
    if should_check():
        ...
        message += [
            "To disable this functionality, set the contents of %s to 'never' (old way)." %
            cache_file()]
        message += [
            "Or add the following lines to ~/.pwn.conf or %s/pwn.conf (or /etc/pwn.conf system-wide):" %
            xdg_config_home]
        message += ["""\
    [update]
    interval=never"""]
