...
from pwntools.tubes.ssh import ssh_channel
from pwntools.tubes.tube import tube
from pwntools.util.fiddling import b64d
...
from pwntools.util.packing import unpack_many
...
class Mapping(object):
    ...
    def __str__(self):
        return '%x-%x %s %x %s' % (self.start, self.stop,
                                   self.permstr, self.size, self.name)
...
class Corefile(ELF):
    ...
    def _parse_nt_file(self, note):
        ...
        for vma, filename in zip(
                note.n_desc.Elf_Nt_File_Entry, note.n_desc.filename):
    ...
    def _describe_core(self):
        ...
        if self.exe and self.exe.name:
            fields += [
                '%-10s %s' % ('Exe:', '%r (%#x)' %
                              (self.exe.name, self.exe.address))
            ]
    ...
    def _parse_stack(self):
        ...
        try:
            if stack[address] != b'\x00':
                log.warning(
                    "Error parsing corefile stack: Could not find end of environment")
                return
        except ValueError:
        ...
        if p_last_env_addr < 0:
            ...
            log.warn_once(
                "Error parsing corefile stack: Found bad environment at %#x",
                last_env_addr)
            return
        ...
        if self.unpack(envp_nullterm) != 0:
            log.warning(
                "Error parsing corefile stack: Could not find end of environment variables")
            return
    ...
    def debug(self):
        ...
        import pwntools.gdb
        pwntools.gdb.attach(self, exe=self.exe.path)
...
class CorefileFinder(object):
    ...
    def native_corefile_pipe(self):
        ...
        if b'/apport' in self.kernel_core_pattern:
            ...
            # Pretend core_pattern was just 'core', and see if we come up with
            # anything
    ...
    def native_corefile_pattern(self):
        ...
        corefile_path = pattern.sub(
            lambda m: replace[re.escape(m.group(0))], core_pattern)
    ...
    def qemu_corefile(self):
        ...
        # Glob all of them, return the *most recent* based on numeric sort
        # order.
