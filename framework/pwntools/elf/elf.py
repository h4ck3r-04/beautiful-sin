...
Stop hard-coding things!  Look them up at runtime with :mod:`pwntools.elf`.
...
from pwntools import adb
from pwntools import qemu
from pwntools.asm import *
from pwntools.context import LocalContext
from pwntools.context import context
from pwntools.elf.config import kernel_configuration
from pwntools.elf.config import parse_kconfig
from pwntools.elf.datatypes import constants
from pwntools.elf.maps import CAT_PROC_MAPS_EXIT
from pwntools.elf.plt import emulate_plt_instructions
...
from pwntools.util.fiddling import unhex
from pwntools.util.misc import align, align_down, which
...
class dotdict(dict):
    ...
    def __getattr__(self, name):
        ...
        subkeys = {k[name_len:]: self[k]
                   for k in self if k.startswith(name_dot)}
...
class ELF(ELFFile):
    ...
    def __init__(self, path, checksec=True):
        ...
        self.statically_linked = bool(
            self.elftype == 'EXEC' and self.load_addr)
    ...
    def debug(self, argv=[], *a, **kw):
        ...
        """
        import pwntools.gdb
        return pwntools.gdb.debug([self.path] + argv, *a, **kw)
    ...
    def iter_segments(self):
        ...
        if self._segments is None:
            self._segments = [
                self.get_segment(i) for i in range(
                    self.num_segments())]
    ...
    def iter_sections(self):
        ...
        if self._sections is None:
            self._sections = [
                self.get_section(i) for i in range(
                    self.num_sections())]
    ...
    def _patch_elf_and_read_maps(self):
        ...
        if sc is None:
            log.error(
                "Cannot patch /proc/self/maps shellcode into %r binary",
                self.arch)
        ...
        try:
            ...
            log.warn_once(
                "Injected /proc/self/maps code did not execute correctly")
            return {}
    ...
    def _populate_plt(self):
        ...
        if self.statically_linked:
            log.debug(
                "%r is statically linked, skipping GOT/PLT symbols" %
                self.path)
            return

        if not self.got:
            log.debug(
                "%r doesn't have any GOT symbols, skipping PLT" %
                self.path)
            return
        ...
        plt_got = self.get_section_by_name(
            '.plt.got')  # <-- Functions used as data
    ...
    def _populate_kernel_version(self):
        ...
        # convert banner into a utf-8 string since re.search does not accept
        # bytes anymore
    ...
    def libc_start_main_return(self):
        ...
        # If there's no delay slot, execution continues on the next instruction
        # after a call.
        ...
        if self.arch in ['arm', 'thumb']:
            ...
            log.error(
                'Unsupported architecture %s in ELF.libc_start_main_return',
                self.arch)
            return 0

        lines = self.functions['__libc_start_main'].disasm().split('\n')
        exit_addr = hex(self.symbols['exit'])
        calls = [(index, line) for index, line in enumerate(
            lines) if set(line.split()) & call_instructions]

        def find_ret_main_addr(lines, calls):
            exit_calls = [
                index for index,
                line in enumerate(calls) if exit_addr in line[1]]
            if len(exit_calls) != 1:
                return 0

            call_to_main = calls[exit_calls[0] - 1]
            return_from_main = lines[call_to_main[0] +
                                     call_return_offset].lstrip()
            return_from_main = int(
                return_from_main[: return_from_main.index(':')], 16)
            return return_from_main

        # Starting with glibc-2.34 calling `main` is split out into
        ...
        # Find a direct call which calls `exit` once. That's probably
        # `__libc_start_call_main`.
        direct_call_pattern = re.compile(
            r'[' + r'|'.join(call_instructions) + r']\s+(0x[0-9a-zA-Z]+)')
        for line in calls:
            ...
            # we might disassemble a bit too much, but it's a good dynamic
            # estimate.
            callee_lines = self.disasm(
                target_addr, self.functions['__libc_start_main'].size).split('\n')
            callee_calls = [(index, line) for index, line in enumerate(
                callee_lines) if set(line.split()) & call_instructions]
    ...
    def vaddr_to_offset(self, address):
        ...
        for interval in self.memory[address]:
            ...
            # Convert the address back to how it was when the segment was
            # loaded
    ...
    def relro(self):
        ...
        >>> path = pwntools.data.elf.relro.path
        >>> for test in glob(os.path.join(path, 'test-*')):
        ...     e = ELF(test)
    ...
    def execstack(self):
        ...
        # If the ``PT_GNU_STACK`` program header is preset, use it's
        # premissions.
        for seg in self.iter_segments_by_type('GNU_STACK'):
    ...
    def stripped(self):
        ...
        return not any(section['sh_type'] ==
                       'SHT_SYMTAB' for section in self.iter_sections())
        ...
        if len(self.config) > 1:
            ...
            for title, values in config_opts.items():
                ...
                for name, message in sorted(values):
                    line = '{} = {}'.format(
                        name, red(str(self.config.get(name, None))))
                    if message:
                        line += ' ({})'.format(message)
                    res.append('    ' + line)
    ...
    def shadowstack(self):
        ...
        for prop in self.iter_properties():
            if prop.pr_type != 'GNU_PROPERTY_X86_FEATURE_1_AND':
                continue
            return prop.pr_data & ENUM_GNU_PROPERTY_X86_FEATURE_1_FLAGS[
    ...
    def set_runpath(exepath, runpath):
        ...
        if not which('patchelf'):
            log.error(
                '"patchelf" tool not installed. See https://github.com/NixOS/patchelf')
            return None
        try:
            subprocess.check_output(
                ['patchelf', '--set-rpath', runpath, exepath], stderr=subprocess.STDOUT)
        except subprocess.CalledProcessError as e:
            log.failure(
                'Patching RUNPATH failed (%d): %r',
                e.returncode,
                e.stdout)
    ...
    def set_interpreter(exepath, interpreter_path):
        ...
        if not which('patchelf'):
            ...
            return None
        try:
            subprocess.check_output(['patchelf',
                                     '--set-interpreter',
                                     interpreter_path,
                                     exepath],
            ...
            log.failure(
                'Patching interpreter failed (%d): %r',
                e.returncode,
                e.stdout)
        return ELF(exepath, checksec=False)

    @staticmethod
    def patch_custom_libraries(
            exe_path, custom_library_path, create_copy=True, suffix='_remotelibc'):
        ...
        interpreter_name = [filename for filename in os.listdir(
            custom_library_path) if filename.startswith('ld-')]
        if interpreter_name:
            interpreter_path = os.path.realpath(
                os.path.join(custom_library_path, interpreter_name[0]))
