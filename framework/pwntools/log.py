...
A pwntools root logger named 'pwntools' is created and a custom handler and
...
case. In particular there are tight loops inside :mod:`pwntools.tubes.tube`, which
...
inside pwntools, that will not even emit a record without :data:`context.log_level`
being set to `logging.DEBUG` or below.

Log records created by ``Progress`` and ``Logger`` objects will set
``'pwntools_msgtype'`` on the ``extra`` field to signal which kind of message was
...
reference to the ``Progress`` logger as ``'pwntools_progress'``.

If the custom handler determines that :data:`term.term_mode` is enabled, log
records that have a ``'pwntools_progess'`` in their ``extra`` field will not
...
``'pwntools.tubes.ssh'`` is set to ``'DEBUG'`` no additional output will show up
unless :data:`context.log_level` is also set to ``'DEBUG'``.  Other handlers
will however see the extra log records generated by the ``'pwntools.tubes.ssh'``
logger.
"""
...
class Logger(object):
  ...
  Also adds some ``pwntools`` flavor:
  ...
  * :meth:`warning_once` (alias :meth:`warn_once`)
  ...
  Adds ``pwntools``-specific information for coloring, indentation and progress
  ...
  def __init__(self, logger=None):
    if logger is None:
      ...
      # from a tube (which do not actually reside in the pwntools library)
  ...
  def _log(self, level, msg, args, kwargs, msgtype, progress = None):
    ...
    extra.setdefault('pwntools_msgtype', msgtype)
    extra.setdefault('pwntools_progress', progress)
    kwargs['extra'] = extra
  ...
  def hexdump(self, message, *args, **kwargs):
    ...
    # TODO: Move pwntools.util.fiddling.hexdump into a new module.
    import pwntools.util.fiddling

    self.info(pwntools.util.fiddling.hexdump(message, *args, **kwargs))
  ...
  def maybe_hexdump(self, message, *args, **kwargs):
    ...
    if len(set(message)) == 1 and len(message) > 1:
      ...
      import pwntools.util.fiddling
      self.indented(pwntools.util.fiddling.hexdump(message), *args, **kwargs)
  ...
  def error(self, message, *args, **kwargs):
    ...
    Logs an error message, then raises a ``pwntoolsException``.
    """
    self._log(logging.ERROR, message, args, kwargs, 'error')
    raise pwntoolsException(message % args)
  ...
  def log(self, level, message, *args, **kwargs):
    ...
    Logs a message with log level `level`.  The ``pwntools`` formatter will
...
class Handler(logging.StreamHandler):
  ...
  An instance of this handler is added to the ``'pwntools'`` logger.
  """
  @property
  def stream(self):
  ...
  def emit(self, record):
    ...
    """
    # We have set the root 'pwntools' logger to have a logLevel of 1,
    ...
    progress = getattr(record, 'pwntools_progress', None)
    ...
    msgtype = record.pwntools_msgtype
    record.pwntools_msgtype = 'animated'
    msg = "%s\n" % self.format(record)
...
class Formatter(logging.Formatter):
  ...
  containing the ``'pwntools_msgtype'`` attribute.  Other records are formatted
  using the `logging` modules default formatter.

  If ``'pwntools_msgtype'`` is set, it performs the following actions:
  ...
  * If the message spans multiple lines they are split, and all subsequent
  ...
  This formatter is used by the handler installed on the ``'pwntools'`` logger.
  """
  ...
  def format(self, record):
    ...
    msgtype = getattr(record, 'pwntools_msgtype', None)

    # if 'pwntools_msgtype' is not set (or set to `None`) we just return the
    # message as it is
    if msgtype is None:
      return msg
...
#
# The root 'pwntools' logger is declared here.  To change the target of all
...
# modified is the root 'pwntools' logger.
#
# For example:
