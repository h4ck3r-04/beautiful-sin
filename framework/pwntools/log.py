...
# `pwnlib.context.log_console.isatty()` is `False`
_msgtype_prefixes = {
...
def read_log_config(settings):
    ...
    for key, value in settings.items():
        if '.' not in key:
            log.warn(
                "Invalid configuration option %r in section %r" %
                (key, 'log'))
            continue
...
class Progress(object):
    ...
    def __init__(self, logger, msg, status, level, args, kwargs):
        ...
        # its status line, so we reset `last_status` to accommodate this
        # pattern
        self.last_status = 0
    ...
    * :meth:`warning_once` (alias :meth:`warn_once`)
    ...
    def __init__(self, logger=None):
        if logger is None:
            ...
            logger_name = '%s.%s.%s' % (
                module, self.__class__.__name__, id(self))
    ...
    def maybe_hexdump(self, message, *args, **kwargs):
        ...
        if len(set(message)) == 1 and len(message) > 1:
            self.indented('%r * %#x' %
                          (message[:1], len(message)), *args, **kwargs)
            ...
            import pwntools.util.fiddling
            self.indented(
                pwntools.util.fiddling.hexdump(message),
                *args,
                **kwargs)
