...
class DynELF(object):
    ...
    memleak vulnerability encapsulated by :class:`pwntools.memleak.MemLeak`.
    ...
    def __init__(self, leak, pointer=None, elf=None, libcdb=True):
        ...
        given a :class:`pwntools.memleak.MemLeak` leaker and a pointer inside
        the binary.

        Arguments:
            leak(MemLeak): Instance of pwntools.memleak.MemLeak for leaking memory
            pointer(int):  A pointer into a loaded ELF file
            elf(str,ELF):  Path to the ELF file on disk, or a loaded :class:`pwntools.elf.ELF`.
        ...
        if elf:
            ...
            self._dynamic = self.elf.get_section_by_name(
                '.dynamic').header.sh_addr
    ...
    def find_base(leak, ptr):
        """Given a :class:`pwntools.memleak.MemLeak` object and a pointer into a
    ...
    def _dynamic_load_dynelf(self, libname):
        ...
        while cur:
            ...
            self.failure(
                "Could not find library with name containing %r" %
                libname)
            return None
    ...
    def _rel_lookup(self, symb, strtab=None, symtab=None, jmprel=None):
        ...
        log.warning(
            "Looking up symbol through DT_JMPREL. This might be slower...")
    ...
    def _lookup(self, symb):
        ...
        def name(tag): return next(
            k for k, v in ENUM_D_TAG.items() if v == tag)
    ...
    def _resolve_symbol_sysv(self, libbase, symb, hshtab, strtab, symtab):
        ...
        while idx != constants.STN_UNDEF:
            ...
            if symtype == constants.STT_FUNC:
                ...
                # Make sure it matches the name of the symbol we were looking
                # for.
                if name == symb:
                    # Bingo
    ...
    def _resolve_symbol_gnu(self, libbase, symb, hshtab, strtab, symtab):
        ...
        # It is set to 1 when a symbol is the last symbol in a given hash
        # chain.
        hsh2 = 0
        while not hsh2 & 1:
            hsh2 = leak.d(chain, i)
